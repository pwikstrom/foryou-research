...
**Annotation backends.** Machine annotation is pluggable: `fyp/annotation/backends/` holds an `AnnotationBackend` ABC (auto-registry, `get_backend()`/`active_backend_name()`) with three implementations — `gemini` (default; thin adapter over the historical `machine_annotation` path), `qwen_local` (Qwen3-Omni-30B via mlx-vlm, Apple Silicon only, frames+audio, llguidance-constrained JSON; mlx-vlm 0.6.x bugs patched in `qwen_rope_fix.py`, upstream Blaizzy/mlx-vlm#1619/#1620; `qwen_api` is a reserved id, not implemented), and `minicpm_local` (MiniCPM-o 4.5 9B, same mlx-vlm frames+audio recipe reused from `qwen_local`'s helpers, ~8 GB peak so it fits 16 GB Macs; the published MLX quants need the checkpoint-naming patch in `minicpm_sanitize_fix.py`; extra `local_minicpm`, checks in `minicpm_support.py`). The **raw-row dict** is the interface boundary — flatten/refine/versioning are backend-agnostic. Backend choice + runtime overrides of the five `[machine]` params (model/temperature/thinking_budget/media_resolution/max_output_tokens) live in the **admin settings store** (Admin → General; `fyp/annotation/backends/settings.py` is the read side, `web_interface/admin_settings.py` the write side); workers apply them at run start via `machine_annotation.apply_admin_machine_overrides()` (snapshots the config baseline, invalidates the cached client/gen-config). **Changing model/params forks a new `av_` annotation version automatically** (model + gen params are hashed into the version identity; the local backend's prompt addendum and frame/audio sampling params fold in too via `extra_params` — additive-only, existing Gemini hashes unchanged). Safety nets: `annotation_configured()` dispatches to the active backend's `availability()` (hardware/deps/model-download checks for qwen via `qwen_support.py`, surfaced in Admin → General's requirements panel, `GET /api/manage/annotation/backends`, `scripts/setup.py --check-only`, and the System-Health annotation chip); the local backend refuses on Cloud Run (`cloud_run_capable=False`, plus a defense-in-depth guard in `process_manager.start_process`); batch mode stays Gemini-only (worker refuses otherwise). ab_eval arms accept optional `backend`/`model`/`temperature` (recorded in the run manifest; absent = production defaults; `mlx-vlm` ships as the `local_qwen` pyproject extra, never in requirements.txt). See docs/installation.md#enabling-local-qwen-annotation.

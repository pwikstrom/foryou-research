...
- **ab_eval** -> annotation_contract, annotation_schema, data_io, fyp_config (L), machine_annotation (L), media_paths (L), recode_variables (L), types
- **activity_analysis** -> logging_setup
- **activity_contract** -> recode_variables (L)
- **activity_versioning** -> activity_contract, data_io (L), logging_setup, registry_metadata (L)
- **annotation_contract** -> data_io (L), logging_setup, recode_variables (L)
- **annotation_schema** -> annotation_contract
- **annotation_versioning** -> annotation_contract (L), annotation_schema (L), data_io (L), fyp_config (L), logging_setup, registry_metadata (L)
- **calc_collection_stats** -> activity_analysis, logging_setup
- **data_io** -> fyp_config (L), logging_setup, types
- **derived_contract** -> recode_variables (L)
- **donations** -> calc_collection_stats, data_io, fyp_config (L), logging_setup, organize_datasets (L), recode_variables
- **embeddings** -> data_io, fyp_config (L), logging_setup
- **fyp_config** -> activity_contract (L), activity_versioning (L), annotation_contract (L), annotation_versioning (L), data_io (L), derived_contract (L), scrape_contract (L), scrape_versioning (L), var_presentation (L)
- **human_eval** -> ab_eval, annotation_contract, data_io, fyp_config (L), logging_setup
- **ingest** -> activity_contract, activity_versioning, data_io, donations, fyp_config (L), logging_setup, organize_datasets (L), polars_ops, recode_variables, scrape_contract, scrape_versioning, structure_sentinel, types, utils
- **instagram_dl** -> fyp_config (L), platform_scraper, scraper_cookies
- **irrelevant_words** -> data_io (L), fyp_config (L), logging_setup
- **machine_annotation** -> annotation_schema, annotation_versioning, data_io, fyp_config (L), logging_setup, media_paths, recode_variables, scrape_queues, types, utils
- **machine_annotation_batch** -> annotation_schema, annotation_versioning, data_io, fyp_config (L), machine_annotation, media_paths
- **media_paths** -> fyp_config (L), scrape_queues (L)
- **organize_datasets** -> annotation_versioning, data_io, fyp_config (L), logging_setup, machine_annotation, polars_ops, recode_variables, scrape, scrape_contract, studies
- **pca** -> data_io, fyp_config (L), logging_setup, organize_datasets, recode_variables, types
- **platform_scraper** -> fyp_config (L), scrape_contract, scrape_versioning (L)
- **polars_ops** -> logging_setup, types
- **recode_variables** -> activity_contract (L), activity_versioning (L), annotation_contract (L), annotation_versioning (L), derived_contract (L), fyp_config (L), irrelevant_words, logging_setup, scrape_contract (L), scrape_versioning (L), types, utils
- **scrape** -> data_io, fyp_config (L), logging_setup, media_paths, platform_scraper, recode_variables, scrape_contract, scrape_queues, scrape_versioning, utils
- **scrape_contract** -> recode_variables (L)
- **scrape_queues** -> data_io (L), logging_setup, scrape_contract (L)
- **scrape_versioning** -> data_io (L), logging_setup, registry_metadata (L), scrape_contract
- **scraper_cookies** -> fyp_config (L)
- **stats** -> logging_setup
- **structure_sentinel** -> data_io, logging_setup, utils
- **studies** -> data_io, fyp_config (L), logging_setup
- **tiktok_dl** -> fyp_config (L), logging_setup, platform_scraper, scraper_cookies
- **timeline_analysis** -> logging_setup
- **types** -> logging_setup
- **utils** -> logging_setup
- **var_presentation** -> data_io (L), logging_setup, scrape_contract
- **video_map** -> data_io, embeddings, fyp_config (L), logging_setup
- **youtube_dl** -> fyp_config (L), platform_scraper, scraper_cookies
...
- **fyp.donations**: 2 file(s)
  ...
  - web_interface/run_collection_metadata_refresh.py
- **fyp.embeddings**: 5 file(s)
  - experiments/embedding_entropy/build_episodes.py
  - experiments/embedding_entropy/data_access.py
  - web_interface/routes/api_semantic_space_routes.py

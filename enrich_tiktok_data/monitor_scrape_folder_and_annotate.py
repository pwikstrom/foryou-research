...
def main() -> None:
    ...
    def mark_processed(path: Path) -> None:
        ...
        print(f"Completed processing {basename(path)}")
    ...
    def handle_new_file(path: Path) -> None:
        ...
        Doing the hard work.
        """
        print(f"Starting annotation of {basename(path)}")
        annotate_from_scrape_metadata_file(cf = cf, scrape_metadata_filename = path, verbose = False)
        print(f"Annotation finished for {basename(path)}")




    #setup_logger()
    print(f"Starting watcher. dir={WATCH_DIR} | prefix={PATTERN_PREFIX} | suffix={PATTERN_SUFFIX} | startup_backlog={PROCESS_STARTUP_BACKLOG}")


    # 1) Load already processed files from the log
    processed = load_processed_files()
    print(f"Loaded {len(processed)} processed files from log")
    existing_files = find_candidate_files()
    print(f"Found {len(existing_files)} existing file(s) in watch dir")
    ...
    if PROCESS_STARTUP_BACKLOG:
        ...
        for path in backlog:
            try:
                ...
                print(f"Error processing backlog file {path}")
    else:

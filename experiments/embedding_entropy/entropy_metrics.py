...
This module is the heart of the *embedding-entropy* experiment: it defines
"how focused / homogeneous is a set of videos" as a property of the raw 1536-d
``gemini-embedding-001`` vectors, **not** of the discrete ``niche`` labels nor
the 2D t-SNE map. The niche-distribution Shannon entropy used by the Semantic
Space trajectory overlay throws away the within-niche geometry; these measures
keep it.

Geometry pipeline (shared by every measure):
    1. **Corpus-mean-centre.** ``gemini-embedding-001`` is anisotropic (all
       vectors share a large common offset, baseline cosine ~0.8), so the raw
       mean is subtracted first — otherwise every pair looks similar.
...
def spectral_entropy(unit_vectors: np.ndarray, weights: np.ndarray | None = None) -> tuple[float, float]:
    ...
    defined and cheap even when ``n`` is far below the 1536 dimensions.
...
def window_metrics(
    ...
    Returns:
        A metrics dict with ``spectral_entropy_bits``, ``effective_rank``,
        ``mean_pairwise_cosine_distance``, ``coherence``, ``dispersion``, and
        ``n_vectors``.
    """

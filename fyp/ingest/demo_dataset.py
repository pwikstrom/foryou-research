"""Synthetic demo-dataset generation (S4 demo study).

Deterministically builds, from a seed: TikTok-DDP-format donor JSONs (parsed
by :class:`~fyp.ingest.tiktok.TikTokDemoCollection`), a contract-conformant
scrape batch written through the real ``_canonicalize_recode_save`` path
(active ``sv_`` stamp), and raw structured machine-annotation entries
(active ``av_`` stamp) consumed by the normal refine/consolidate path.

All item ids are 19 digits starting with ``utils.DEMO_ITEM_ID_PREFIX`` so
demo material stays out of corpus-global artifacts (embeddings map) and the
scrape/annotation queue builders. The content model plants real group
differences (persona niche weights, per-niche political/sensitivity/gender/
advertising distributions, completion behaviour) so every analysis tab shows
teachable structure.

Callers: ``scripts/generate_demo_dataset.py`` (CLI) and
``web_interface/run_demo_dataset.py`` (the one-click admin worker).
"""
...
DEMO_STUDY_NAME = "Demo study (synthetic data)"




def ensure_demo_study(collection_ids: list[str], days: int = DEFAULT_DAYS,
                      as_of: str = DEFAULT_AS_OF) -> bool:
    """Create the demo study definition if it does not exist yet.

    Shared with everyone (``USER_ACCESS = ['all']``), flagged ``SYNTHETIC`` so
    the methods note and Explore banner announce the provenance, sampling off.
    An existing definition is left untouched (an admin may have customised it).

    Args:
        collection_ids: The demo collection ids (``DEMO_P01``...).
        days: The generated window length (sets START_DATE).
        as_of: The generator's anchor date (sets END_DATE).

    Returns:
        True when a new definition was created and saved.
    """
    from fyp.studies import init_study_defs, save_study_defs, _cf

    init_study_defs()
    defs = _cf()["study_defs"]
    if DEMO_STUDY_NAME in defs:
        return False

    anchor = dt.datetime.strptime(as_of, "%Y-%m-%d")
    start = (anchor - dt.timedelta(days=days + 1)).strftime("%Y-%m-%d")
    defs[DEMO_STUDY_NAME] = {
        "STUDY_NAME": DEMO_STUDY_NAME,
        "SYNTHETIC": True,
        ...
        "START_DATE": start,
        "END_DATE": as_of,
        "MIN_ACTIVITY_COUNT_PER_GROUP": 0,
        "MAX_ACTIVITY_COUNT_PER_GROUP": 0,
        "MIN_GROUP_COUNT_PER_COLLECTION": 0,
        "MAX_GROUP_COUNT_PER_COLLECTION": 0,
    }
    save_study_defs()
    return True

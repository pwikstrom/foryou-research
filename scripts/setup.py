...
dependencies — the wizard itself is stdlib-only:
    ...
    python scripts/setup.py --gcs-bucket my-bucket --gcs-for media --yes
...
# The three independently switchable storage surfaces (data_io.use_gcs_for_*).
...
class Answers:
    ...
    gemini_api_key: str = ""
    gcs_data: bool = False
    gcs_media: bool = False
    gcs_cache: bool = False
    gcs_bucket: str = ""
    contact_email: str = ""
    flask_secret: str = ""
    ...
    @property
    def gcs(self) -> bool:
        """True when any surface is on GCS — i.e. when a bucket is needed."""
        return self.gcs_data or self.gcs_media or self.gcs_cache

    @property
    def needs_local_data(self) -> bool:
        """True when ``paths.local_data`` still holds real files.

        The cache directory is derived from it (``fyp_config`` sets
        ``paths.cache = local_data/cache``), so a local cache keeps the data
        path meaningful even when the data itself lives in the bucket.
        """
        return not (self.gcs_data and self.gcs_cache)

    @property
    def needs_local_media(self) -> bool:
        """True when media files are stored on this machine's disk."""
...
def dependencies_installed() -> bool:
    ...
    Probes two third-party imports that only ``pip install -r
    requirements-dev.txt`` provides, so a bare ``.venv`` with nothing in it
    still reads as "not installed".
    """
    for module in ("flask", "pandas"):
        try:
...
def build_config_toml(answers: Answers, generated_on: str = "") -> str:
    ...
    # Only the paths that still hold files are overridden: a fully
    # bucket-backed install has no local directory to name, and the committed
    # paths.local_data then serves purely as the template the GCS prefixes are
    # derived from (fyp_config builds gcs_paths by relpath against it).
    path_lines = []
    if answers.needs_local_data:
        path_lines.append(f'local_data = "{answers.data_dir}"')
    if answers.needs_local_media:
        path_lines.append(f'local_media = "{resolve_media_dir(answers)}"')
    if path_lines:
        lines += ["", "[paths]", *path_lines]
    else:
        lines += [
            "",
            "# Data, media and cache are all in the bucket, so no local path is",
            "# overridden here - paths.local_data in config/config.toml is used",
            "# only as the template the bucket prefixes are derived from.",
        ]

    lines += ["", "[data_io]"]
    lines += [
        f"use_gcs_for_data = {'true' if answers.gcs_data else 'false'}",
        f"use_gcs_for_media = {'true' if answers.gcs_media else 'false'}",
        f"use_gcs_for_cache = {'true' if answers.gcs_cache else 'false'}",
    ]
...
def check_environment(include_local_models: bool = False) -> list[CheckResult]:
    ...
    """
    deps_ok = dependencies_installed()
    results = [
        CheckResult(
            name="python",
        ...
        # Informational: running the wizard before pip install is a supported
        # order, so "not installed yet" is a normal state, not a failure.
        CheckResult(
            name="dependencies",
            ok=deps_ok,
...
def load_existing_defaults() -> Answers:
    ...
    # Read each surface on its own: a hand-edited mixed overlay (say, media
    # on GCS and everything else local) must come back as itself, not be
    # flattened to whatever use_gcs_for_data happens to say.
    answers.gcs_data = bool(data_io_cfg.get("use_gcs_for_data", False))
    answers.gcs_media = bool(data_io_cfg.get("use_gcs_for_media", False))
    answers.gcs_cache = bool(data_io_cfg.get("use_gcs_for_cache", False))
...
def ask_storage(answers: Answers, defaults: Answers) -> None:
    """Ask the storage questions, filling ``answers`` in place.

    GCS is asked first, per surface, because the answers decide which local
    directories are still needed — a fully bucket-backed install is never
    asked for a path it would not use.
    """
    print("\nStorage - where data, media and cache files live:")
    print("  Local disk is the default. Google Cloud Storage needs a bucket you")
    print("  can already write to (gcloud auth application-default login).")
    if prompt_yes_no("Store anything in Google Cloud Storage?", defaults.gcs):
        answers.gcs_bucket = prompt("GCS bucket name", defaults.gcs_bucket)
        print("  Each surface is independent - answer them separately:")
        # A fresh opt-in means all three; re-running over an existing overlay
        # offers back exactly what that overlay says.
        mixed = defaults.gcs
        answers.gcs_data = prompt_yes_no(
            "  Data on GCS? (activity data, annotations, scrape records)",
            defaults.gcs_data if mixed else True,
        )
        answers.gcs_media = prompt_yes_no(
            "  Media on GCS? (the video files)",
            defaults.gcs_media if mixed else True,
        )
        answers.gcs_cache = prompt_yes_no(
            "  Cache on GCS? (derived analysis artifacts, rebuildable)",
            defaults.gcs_cache if mixed else True,
        )
        if not answers.gcs:
            print("  Nothing selected - all three stay on the local disk.")
            answers.gcs_bucket = ""

    if answers.needs_local_data:
        label = (
            "Data directory (all project data except media)"
            if not answers.gcs_data
            else "Data directory (only the local cache lives here)"
        )
        while True:
            raw = prompt(label, defaults.data_dir)
...
def gcs_surfaces_from_args(args: argparse.Namespace, defaults: Answers) -> Answers:
    """Resolve the three GCS surface flags + bucket from CLI flags + defaults.

    ``--gcs-bucket`` alone means all three surfaces (the common "put
    everything in the bucket" case); ``--gcs-for`` narrows that to a subset.
    With neither flag, an existing overlay's per-surface values carry over
    untouched.
    """
    answers = Answers()
    if not (args.gcs_bucket or args.gcs_for):
        answers.gcs_data = defaults.gcs_data
        answers.gcs_media = defaults.gcs_media
        answers.gcs_cache = defaults.gcs_cache
        answers.gcs_bucket = defaults.gcs_bucket
        return answers

    answers.gcs_bucket = args.gcs_bucket or defaults.gcs_bucket
    if not answers.gcs_bucket:
        sys.exit("error: --gcs-for needs a bucket - add --gcs-bucket NAME")
    if args.gcs_for:
        wanted = {s.strip() for s in args.gcs_for.split(",") if s.strip()}
        unknown = wanted - set(GCS_SURFACES)
        if unknown:
            sys.exit(
                f"error: --gcs-for: unknown surface(s) {', '.join(sorted(unknown))} "
                f"(choose from {', '.join(GCS_SURFACES)})"
            )
    else:
        wanted = set(GCS_SURFACES)
    answers.gcs_data = "data" in wanted
    answers.gcs_media = "media" in wanted
    answers.gcs_cache = "cache" in wanted
    return answers
...
def answers_from_args(args: argparse.Namespace, defaults: Answers) -> Answers:
    ...
    # Storage first, as in the interactive flow: a local path that nothing
    # will use is neither validated nor written.
    if answers.needs_local_data:
...
def print_next_steps(answers: Answers, in_venv: bool, deps_ready: bool = False) -> None:
    ...
    ``deps_ready`` suppresses the venv/pip steps — set when this run installed
    them or when they were already installed before the wizard started.
    """
    windows = os.name == "nt"
...
def main() -> None:
    ...
    # type them; --install skips the question. Anyone who followed
    # docs/installation.md is already in a venv with the dependencies in it,

...
def pinned_var_schema(schema_csv: Path = SCHEMA_SNAPSHOT):
    ...
    try:
        if Path(schema_csv).exists():
            ...
            # Scrape metadata + accepted_labels are contract-owned and not stored in
            # the CSV/snapshot; rebuild them in memory exactly as the live load path
            # does (scrape metadata first, mirroring load_var_schema).
            _apply_contract_scrape_metadata(fyp_cf)

...
def build_interpretation(study: str) -> tuple[str | None, str | None]:
    ...
    prompt = (
        "You are helping a researcher read a statistics page. The data: each "
        "observation is a 'collection-day' — all videos in one collection (a set of "
        "timestamped video-feed records, not necessarily one person) on one calendar "
        "day, averaged. Variables come from AI annotation of video content, scraped "
        "video metadata, and viewing activity. '(C0)' etc. are PCA components of a "
        ...
        "the collections in this study differ from each other and on what; 1-2 "
        "sentences on what moves within these collections; one closing sentence on "
        "limits.\n\n"
        "SCOPE — the most important rule: this is a convenience sample of specific "
        "collections, NOT a representative sample of any population or platform. "
        "Every claim must stay bound to this dataset: write 'the collections in this "
        "study', 'in this dataset', 'these feeds' — never 'social media', 'users', "
        "'people', or 'feeds' in general, and never present a pattern as how a "
        "platform or its users behave. Do not open with a generalizing sentence; "
        "state the scope in the first sentence, not only at the end.\n\n"
        "Further rules: no causal claims (associations only); days within a "
        "collection are not independent, so treat significance as descriptive; do "
        "not invent numbers not present below; no markdown headings or bullet lists "
        "— flowing prose only.\n\n"
        f"FINDINGS\n{findings}"
    )
